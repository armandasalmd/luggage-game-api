...
export default class HistoryUseCase
  implements IUseCase<HistoryRequest, HistoryResponse> {
  async execute(
            ...
            isSurrendered: {
              $anyElementTrue: {
                $filter: {
                  ...
                  as: "p",
                  cond: {
                    $eq: ["$$p.playerState", GameUtils.PlayerState.Surrendered],
                  },
                },
              },
            },
  ...
  private toHistoryItem(history: IAggregatedPlayerHistory): IHistoryItem {
    ...
    return {
      ...
      place: history.isSurrendered ? "Surrendered" : history.place,
